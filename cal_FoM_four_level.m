function [F,G] = cal_FoM_four_level(T, grad_adjs)
...
% 
% F = -sum(T,'all') + 1.5*(T(1,1)+T(2,2)+T(3,4)+T(4,5));
% G = squeeze(-sum(sum(grad_adjs,1),2) +1.5*(grad_adjs(1,1,:)+grad_adjs(2,2,:)+grad_adjs(3,4,:)+grad_adjs(4,5,:)));
